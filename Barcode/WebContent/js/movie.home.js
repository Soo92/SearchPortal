...
sHTML=template.auto_people_contents.replace(/{{title}}/g,value[0][0]).replace("{{title_match}}",self.makeAutoText(query,value[0][0])).replace("{{time}}",(value[1][0]!=0)?value[1][0]:"").replace("{{movie}}",(value[2][0])?"<dd>"+value[2][0]+"</dd>":"").replace("{{thumbnail}}",(value[3][0])?"http://search.de.phinf.net/common/?src="+escapedImageSrc+"&type=u120_150&quality=95":"http://static.naver.net/movie/2012/06/dft_img30x48.png").replace("{{link}}",options.peoplelink+value[5][0]);
break
}return sHTML
